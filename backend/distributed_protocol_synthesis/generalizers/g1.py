...
class G1(G0):
    ...
    def _generalize(self, candidate):
        ...
        if is_correct and True:
            fixed_dict = dict()
            dead_transitions = self._main_product.dead_automata_transitions()
            for k,v in dead_transitions.items():
                fixed_dict[k.name] = v
            dead_transitions = fixed_dict

            relevant_dead_transitions = list()
            relevant_nondead_transitions = list()
            for proc, p, a, q in new_transitions:
                b1 = proc in list(dead_transitions.keys())
                b2 = b1 and ((p,a,q) in dead_transitions[proc])
                if b2:
                    relevant_dead_transitions.append((proc,p,a,q))
                else:
                    relevant_nondead_transitions.append((proc,p,a,q))

            if len(relevant_dead_transitions) != 0:
                results.append({"disable" : relevant_nondead_transitions})
